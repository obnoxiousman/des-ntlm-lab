...
void ntlm_make_des_key(unsigned char *key56, unsigned char *key) {
    key[0] = key56[0] >> 1;
    key[1] = ((key56[0] & 0x01) << 6) | (key56[1] >> 2);
    key[2] = ((key56[1] & 0x03) << 5) | (key56[2] >> 3);
    key[3] = ((key56[2] & 0x07) << 4) | (key56[3] >> 4);
    key[4] = ((key56[3] & 0x0F) << 3) | (key56[4] >> 5);
    key[5] = ((key56[4] & 0x1F) << 2) | (key56[5] >> 6);
    key[6] = ((key56[5] & 0x3F) << 1) | (key56[6] >> 7);
    key[7] = (key56[6] & 0x7F) << 1;

    for (int i = 0; i < 8; i++) {
        unsigned char b = key[i] & 0xFE;
        unsigned char parity = 0;
        for (int j = 0; j < 7; j++) {
            parity ^= ((b >> j) & 1);
        }
        key[i] = b | (parity == 0 ? 1 : 0);
    }
}
